 ...
 *
 * Everything upstream - INCOIS advisories, the drift model, the trip planner -
 * works in nautical miles, which is right for a chart and wrong for the person
 * reading it. A working fisherman thinks in kilometres, and "NM" on a wet phone
 * screen reads as nothing at all. Convert at the point of display, never in the
 * data.
 */
export function nmToKm(nauticalMiles) {
  if (nauticalMiles == null || !Number.isFinite(Number(nauticalMiles))) return null;
  return Math.round(Number(nauticalMiles) * 1.852 * 10) / 10;
}

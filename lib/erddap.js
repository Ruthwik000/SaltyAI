 ...
 */
export const KNOWN_INCOIS_DATASETS = [
...
export const KNOWN_INCOIS_VARIABLES = {
...
export function datasetJsonUrl(id) {
...
export async function fetchErddapCatalogue(signal) {
  try {
    ...
    const col = (name) => table.columnNames.indexOf(name);
    ...
    return {
      ...
      live: false,
      reason: error instanceof Error ? error.message : "Unable to reach INCOIS ERDDAP",
    };
  }
}
 ...
 */
export async function fetchDatasetVariables(id, signal) {
  try {
 ...
 */
export function inferDatasetFacets(id) {
 ...
 */
export async function fetchErddapPointSeries(options, signal) {
  ...
  if (timeAt < 0 || valueAt < 0)
    throw new Error("Griddap response has no time/value column");
  ...
  if (points.length === 0)
    throw new Error("No values returned for this point and window");

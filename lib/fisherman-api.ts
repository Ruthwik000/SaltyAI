...
export interface Sourced<T> {
  data: T;
  source: DataSource;
  /** Present only when the backend call failed and demo data was used. */
  reason?: string;
}
...
export interface PfzZoneFeature {
  ...
  lat: number;
  lon: number;
  /** Advisory circle radius drawn on the map, in nautical miles. */
  radiusNM: number;
...
export type BoatType = "craft" | "motorized" | "trawler" | "longliner";

export interface TripRiskRequest {
...
export interface TripRiskResult {
  score: number;
...
export function backendIsCoolingDown(): boolean {
...
function toFeature(zone: PFZZone, from?: LatLon): PfzZoneFeature {
...
function demoZonesNear(from: LatLon): PfzZoneFeature[] {
...
function conditionsFromLocation(location: MarineLocation): PointConditions {
  return {
    sst: location.sst,
...
function nearestLocation(lat: number, lon: number): MarineLocation {
  return marineLocations.reduce((closest, candidate) => {
    const d = (a: MarineLocation) =>
      (a.lat - lat) ** 2 + (a.lon - lon) ** 2;
    return d(candidate) < d(closest) ? candidate : closest;
  }, marineLocations[0]);
}

function demoZoneDetail(zoneId: string): ZoneDetail | null {
...
function demoAlerts(): OceanAlert[] {
 ...
 */
const BOAT_EXPOSURE: Record<BoatType, { label: string; factor: number }> = {
...
function nightHours(fromIso: string, toIso: string): number {
...
function demoTripRisk(request: TripRiskRequest): TripRiskResult {
  ...
  const level: TripRiskResult["level"] =
  ...
  if (score < 30) {
    ...
    recommendations.push("Consider an earlier departure so you are back before conditions build.");
  }
  ...
  return {
    ...
    factors: [
      { name: "Sea state", value: `${near.waveHeight} m at ${near.wavePeriod} s`, score: seaState },
      { name: "Wind", value: `${near.windSpeed} kts ${near.windDirection}`, score: wind },
      { name: "Swell", value: `${near.swellHeight} m`, score: swell },
      { name: "Crossing distance", value: `${crossing.toFixed(1)} NM each way`, score: range },
...
export async function fetchPfzZones(
  ...
  lon: number,
  signal?: AbortSignal
): Promise<Sourced<PfzZoneFeature[]>> {
  try {
    const data = await call<PfzZoneFeature[]>(
      `/api/fisherman/zones?lat=${lat}&lon=${lon}`,
      { signal }
    );
...
export async function fetchZoneDetail(
  zoneId: string,
  signal?: AbortSignal
): Promise<Sourced<ZoneDetail | null>> {
  try {
    const data = await call<ZoneDetail>(
      `/api/fisherman/zones/${encodeURIComponent(zoneId)}`,
      { signal }
    );
    return { data, source: "live" };
  } catch (error) {
...
export async function fetchPointConditions(
  ...
  lon: number,
  signal?: AbortSignal
): Promise<Sourced<PointConditions>> {
  try {
    const data = await call<PointConditions>(
      `/api/fisherman/conditions?lat=${lat}&lon=${lon}`,
      { signal }
    );
    return { data, source: "live" };
  } catch (error) {
...
export async function fetchOceanAlerts(
  lat: number,
  lon: number,
  signal?: AbortSignal
): Promise<Sourced<OceanAlert[]>> {
  try {
    const data = await call<OceanAlert[]>(
      `/api/fisherman/alerts?lat=${lat}&lon=${lon}`,
      { signal }
    );
    return { data, source: "live" };
  } catch (error) {
...
export async function assessTripRisk(
  request: TripRiskRequest,
  signal?: AbortSignal
): Promise<Sourced<TripRiskResult | null>> {
  try {
    const data = await call<TripRiskResult>("/api/fisherman/risk/assess", {
      method: "POST",
...
export async function startTrip(
  ...
  signal?: AbortSignal
): Promise<Sourced<TripSession>> {
  const local: TripSession = {
  ...
  try {
    const data = await call<TripSession>("/api/fisherman/trip/start", {
      method: "POST",
      body: JSON.stringify(request),
      signal,
    });
...
export async function pushTripPing(
  tripId: string,
  point: TrackPoint
): Promise<boolean> {
  try {
    await call<unknown>(
      `/api/fisherman/trip/${encodeURIComponent(tripId)}/ping`,
...
export async function endTrip(tripId: string): Promise<boolean> {
  try {
    await call<unknown>(
      `/api/fisherman/trip/${encodeURIComponent(tripId)}/end`,
      { method: "POST", body: JSON.stringify({ endedAt: new Date().toISOString() }) }
    );
    return true;
  } catch {
    return false;

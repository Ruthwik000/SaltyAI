...
export async function checkCallAgent() {
  ...
  if (result.status !== "ready")
    throw new Error("Exotel is not configured on the Call Agent");
  return result;
}

export async function startExotelCall(phoneNumber) {
...
function isUnsupportedAgentRefusal(response) {
  return /\btools?\b.*\b(?:do not|don't|not|no)\b.*\b(?:functions?|weather|sail(?:ing)?|safety)\b|\b(?:cannot|can't|unable)\b.*\b(?:determine|answer)\b.*\b(?:fishing|sail(?:ing)?|weather|safety)\b/i.test(
    response
  );
}

function mockAgentResponse(query) {
  const safetyQuestion =
    /\b(?:safe|safety|sail|sailing|boat|fishing|fish|departure|hazard|danger|risk)\b/i.test(
      query
    );
  if (safetyQuestion) {

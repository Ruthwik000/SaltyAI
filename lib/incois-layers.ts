...
export type OsfLayerKey = keyof typeof osfLayers;

export interface DiscoveredOsfFiles {
  sst: string;
  currents: string;
  waves: string;
  mld: string;
...
export function osfDatasetPath(
  key: OsfLayerKey,
  files?: DiscoveredOsfFiles | null
): string {
 ...
 */
export function osfFeatureInfoUrl(
  key: OsfLayerKey,
  lat: number,
  lon: number,
  files?: DiscoveredOsfFiles | null,

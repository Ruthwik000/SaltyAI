...
/**
 * India EEZ boundary, as vector geometry.
 *
 * Discovered from the official PFZ WebGIS at
 * incois.gov.in/DataInfo/MFASPFZ/index.html, which draws this layer as WMS
 * tiles. The same GeoServer answers WFS, and the EEZ comes back as 19
 * MultiLineString features in ~29 KB - small enough to draw as a real line
 * rather than a picture.
 *
 * It has to be vector: GeoServer here ignores SLD_BODY (dynamic styling is
 * off), so a WMS request cannot be recoloured - the tile always renders in
 * the server's own style. Vector is the only way to draw it red.
 *
 * Note the workspace prefix is PFZ_EEZ:indiaeez for WFS, even though the WMS
 * request for the same layer uses PFZ_Automation:indiaeez. Asking WFS for the
 * PFZ_Automation prefix returns "Feature type unknown".
 */
export const eezGeoJsonUrl =
  "/api/incois/frame/geoserver/PFZ_EEZ/ows" +
  "?service=WFS&version=1.0.0&request=GetFeature" +
  "&typeName=PFZ_EEZ%3Aindiaeez&outputFormat=application%2Fjson";

/** Maritime boundary red. Deliberately the only red on the chart. */
export const EEZ_LINE_COLOUR = "#e5484d";
...
import { liveAlerts, liveConditions, liveForecast, liveZoneDetail, liveZones } from "./live-data";
...
export async function fetchPfzZones(lat, lon, signal) {
  try {
    const data = await liveZones(lat, lon, signal);
...
export async function fetchZoneDetail(zoneId, signal) {
  try {
    const data = await liveZoneDetail(zoneId, signal);
    return { data, source: "live" };
  } catch (error) {
    return {
...
export async function fetchPointConditions(lat, lon, signal) {
  try {
    const data = await liveConditions(lat, lon, signal);
    return { data, source: "live" };
  } catch (error) {
    return {
...
export async function fetchOceanAlerts(lat, lon, signal) {
  try {
    const data = await liveAlerts(nearestLocation(lat, lon).state, signal);
    return { data: data.map((alert) => normalizeAlert(alert, "live")), source: "live" };
  } catch (error) {
    return {
      data: demoAlerts().map((alert) => normalizeAlert(alert, "demo")),
      source: "demo",
      reason: describe(error),
    };
  }
...
export async function fetchForecast(lat, lon, signal) {
  try {
    const data = await liveForecast(lat, lon, signal);
    return { data, source: "live" };
  } catch (error) {

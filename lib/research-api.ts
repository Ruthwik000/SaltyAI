...
export interface SeriesStats {
  ...
  max: number;
  stdDev: number;
  /** Ordinary least squares slope, expressed per decade. */
  trendPerDecade: number;
  /** Mean departure from the monthly climatology over the window. */
  anomalyMean: number;
}
...
export interface ResearchSeries {
  ...
  points: SeriesPoint[];
  /** Monthly climatology expanded across the window, for the baseline line. */
  baseline: SeriesPoint[];
  climatology: MonthlyMean[];
...
const UNIT_RULES: { match: RegExp; unit: string; base: number; swing: number }[] = [
...
function unitRuleFor(variable: string) {
  return (
...
export function unitForVariable(variable: string): string {
...
function trendPerDecade(values: number[]): number {
...
function buildHistogram(values: number[], bins = 10): HistogramBin[] {
...
export function buildSeriesFromPoints(
  ...
  const climatology: MonthlyMean[] = byMonth.map((values, index) => ({
    month: MONTH_NAMES[index],
    mean: Number(mean(values.length ? values : [overall]).toFixed(3)),
  }));

  const baseline: SeriesPoint[] = points.map((point) => ({
    t: point.t,
...
function demoSeries(request: {
  ...
  const seasonal = (monthIndex: number) =>
    rule.base + rule.swing * Math.sin((monthIndex / 12) * Math.PI * 2 + phase);

  const points: SeriesPoint[] = months.map((date, index) => {
    const years = index / 12;
    const value =
      seasonal(date.getMonth()) +
      drift * years +
      (random() - 0.5) * 2 * noiseScale;
    return {
...
export async function fetchResearchSeries(
  ...
  signal?: AbortSignal
): Promise<Sourced<ResearchSeries>> {
  ...
  try {
    ...
    return {
      ...
      source: "demo",
      reason:
        error instanceof Error ? error.message : "SALTY backend unreachable",
    };
  } finally {
    clearTimeout(timer);
  }

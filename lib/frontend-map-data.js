...
function fieldRecords(parameter) {
  ...
  for (let latIndex = 0; latIndex < 7; latIndex += 1) {
    for (let lonIndex = 0; lonIndex < 9; lonIndex += 1) {
      ...
      const wave =
        Math.sin((latIndex + 1) * 1.4) * 0.45 + Math.cos((lonIndex + 2) * 0.8) * 0.35;
      records.push({
        latitude,
        longitude,
...
export const frontendMapLayers = {
  region: { min_lat: 17, max_lat: 18.5, min_lon: 82.5, max_lon: 84.5 },
  start: "frontend",
  end: "frontend",
  layers: Object.fromEntries(
    Object.keys(fields).map((parameter) => [parameter, fieldRecords(parameter)])
  ),

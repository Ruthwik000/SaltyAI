...
export async function GET(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  try {
    ...
    if (type.includes("text/html") || type.includes("javascript") || type.includes("css")) {
      ...
      return new NextResponse(body, { status: upstream.status, headers: { "Content-Type": type, "Cache-Control": "no-store" } });
    }
    return new NextResponse(await upstream.arrayBuffer(), { status: upstream.status, headers: { "Content-Type": type, "Cache-Control": "no-store" } });
  } catch {

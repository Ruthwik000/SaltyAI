...
import { fetchPfzZones, type PfzZoneFeature } from "@/lib/fisherman-api";
import { researchOsfLayers, type OsfLayerKey } from "@/lib/incois-layers";
...
type MapView = "official" | "research" | "operations";
...
export default function MarineMapPage() {
  ...
  const [view, setView] = React.useState<MapView>(
    role === "operator" ? "operations" : "official"
  );
  const [overlay, setOverlay] = React.useState<OsfLayerKey | null>("sst");
    ...
    <div className="-mx-4 -mt-4 flex h-[calc(100dvh-8.5rem)] flex-col sm:-mx-6 sm:-mt-6 lg:-mx-8 lg:-mt-8 lg:h-[calc(100dvh-4.5rem)]">
        ...
        <div className="flex overflow-x-auto rounded-lg border border-zinc-200">
          {(isOperator
            ? ([
              ...
              ] as { id: MapView; label: string; icon: React.ElementType }[])
            : ([
                { id: "official", label: "INCOIS forecast", icon: Globe2 },
      ...
      {view === "research" && (
        <p className="shrink-0 border-t border-zinc-200 bg-white px-4 py-1.5 text-[10px] leading-snug text-zinc-500">
          Variables read from the INCOIS Ocean State Forecast WMS. Tap the map to
          read the active variable at that point. Ocean-colour products
          (chlorophyll, Kd490) are not wired yet — their service request has to be
          captured from the official PFZ interface first.
        </p>
      )}
    </div>
  );

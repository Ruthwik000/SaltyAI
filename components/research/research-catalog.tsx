...
interface CatalogDataset {
  ...
  dimensions?: string[];
  timeCoverage?: { time_coverage_start?: string; time_coverage_end?: string };
  geospatial?: { latMin: number; latMax: number; lonMin: number; lonMax: number; latRes: number; lonRes: number };
  error?: string;
}
...
function iconFor(variable?: string) {
...
export function ResearchCatalog() {
  const [datasets, setDatasets] = React.useState<CatalogDataset[] | null>(null);
...
function DatasetDetail({ dataset }: { dataset: CatalogDataset }) {
  const [series, setSeries] = React.useState<SeriesPoint[] | null>(null);
  const [seriesError, setSeriesError] = React.useState<string | null>(null);
  const [point, setPoint] = React.useState<{ lat: number; lon: number } | null>(null);

  const [frames, setFrames] = React.useState<string[] | null>(null);
  const [framesError, setFramesError] = React.useState<string | null>(null);
...
function AnimationPlayer({ datasetId, frames }: { datasetId: string; frames: string[] }) {

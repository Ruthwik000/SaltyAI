...
function OriginBadge({ origin, reason }) {
...
const REGION_PRESETS = [
  {
    id: "bob",
    label: "Bay of Bengal",
    bounds: { minLat: 8, maxLat: 22, minLon: 80, maxLon: 94 },
  },
  {
    id: "as",
    label: "Arabian Sea",
    bounds: { minLat: 8, maxLat: 24, minLon: 60, maxLon: 76 },
  },
  {
    id: "eez",
    label: "Indian EEZ",
    bounds: { minLat: 6, maxLat: 24, minLon: 60, maxLon: 94 },
  },
  {
    id: "ap",
    label: "Andhra shelf",
    bounds: { minLat: 15, maxLat: 19.5, minLon: 82, maxLon: 86 },
  },
];
...
const startOfMonth = (value) => `${value}-01`;
...
export function ResearchConsole() {
  ...
  const [catalogueReason, setCatalogueReason] = React.useState();
  ...
  const [variablesById, setVariablesById] = React.useState({});
  const [variablesFailed, setVariablesFailed] = React.useState({});
  ...
  const [region, setRegion] = React.useState(REGION_PRESETS[0].bounds);
  ...
  const [origin, setOrigin] = React.useState("demo");
  const [pointAt, setPointAt] = React.useState(null);
  const [reason, setReason] = React.useState();
  const [copied, setCopied] = React.useState(false);

  const requestKey =
    dataset && activeVariable
    ...
    <div className="space-y-5">
      <header className="flex flex-col gap-3 border-b border-zinc-200 pb-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          ...
          >
            <Star
              className={`h-3.5 w-3.5 ${starredOnly ? "fill-amber-500 text-amber-500" : ""}`}
            />
            <span>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,300px)_minmax(0,1fr)] xl:grid-cols-[minmax(0,340px)_minmax(0,1fr)]">
        ...
        <aside className="space-y-3">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            >
              <Star
                className={`h-3 w-3 ${starredOnly ? "fill-amber-500 text-amber-500" : ""}`}
              />
                ...
                {variablesPending ? (
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[10px] leading-snug text-zinc-400">
                      This dataset&rsquo;s variable list could not be read from ERDDAP.
                      Type the variable name as it appears on its ERDDAP page.
                    </p>
                  </div>
                )}
              </div>
          ...
          {!activeVariable ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-6">
                <StatTile
                  label="Mean"
                  value={`${series.stats.mean}${unit ? ` ${unit}` : ""}`}
                />
              ...
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                  ...
                  table={
                    <ValueTable
                      ...
                      rows={series.points.map((point, index) => [
                        ...
                        Number(
                          (
                            point.value - (series.baseline[index]?.value ?? point.value)
                          ).toFixed(3)
                        ),
                      ])}
                    />
                ...
                >
                  <BarChart
                    bars={series.climatology.map((item) => ({
                      label: item.month,
                      value: item.mean,
                    }))}
                    unit={unit}
              ...
              <section className="rounded-xl border border-zinc-200 bg-white p-4">
                ...
                <p className="mt-0.5 text-[11px] text-zinc-500">
                  Griddap request for the dataset, variable, region and window selected
                  above.
                </p>
                ...
                <p className="mt-2 text-[10px] leading-snug text-zinc-400">
                  Downloads cover the whole region box and are fetched through SALTY; the
                  URL above is the public one for your own tooling. Axis order assumes the
                  usual griddap layout of time, latitude, longitude — a dataset with
                  different dimensions needs its own order, shown on its ERDDAP page.
                </p>
              </section>
            </>
          )}

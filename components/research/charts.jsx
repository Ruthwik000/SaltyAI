...
export function TimeSeriesChart({ points, baseline, unit, height = 230 }) {
    ...
    <div ref={ref} className="relative w-full">
      ...
      {active && (
        <div
          ...
          style={{ left: Math.min(Math.max(x(hover) - 40, 0), Math.max(0, width - 96)) }}
        >
...
export function AnomalyChart({ points, baseline, unit, height = 170 }) {
  const { ref, width } = useWidth();
  const [hover, setHover] = React.useState(null);
  ...
  const y = (v) => M.top + innerH / 2 - (v / extent) * (innerH / 2);
  ...
  return (
    <div ref={ref} className="relative w-full">
      <svg
        ...
        height={height}
        role="img"
        aria-label="Departure from climatology"
        onMouseLeave={() => setHover(null)}
      >
        <line
          x1={M.left}
      ...
      {hover != null && (
        <div
          ...
          style={{
            left: Math.min(
              Math.max(M.left + hover * slot - 30, 0),
...
export function BarChart({ bars, unit, height = 170, valueLabel }) {
  const { ref, width } = useWidth();
  const [hover, setHover] = React.useState(null);
  ...
  const span = hi - lo || 1;
  const y = (v) => M.top + innerH - ((v - lo) / span) * innerH;

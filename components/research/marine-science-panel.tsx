...
export function wavePowerKwPerM(hsMetres: number, peakPeriodS: number): number {
...
export function windStressNm2(windKnots: number): number {
 ...
 */
export function ekmanTransport(windKnots: number, latDeg: number): number {
...
export function douglasSeaState(hs: number): { code: number; label: string } {
 ...
 */
const WINDOW_OFFSETS: { hour: string; wind: number; wave: number; swell: number; temp: number }[] = [
...
export function buildForecastHours(location: MarineLocation): ForecastHour[] {
...
export function MarineSciencePanel({
  location,
  hours,
  variant = "full",
}: {
  ...
  hours: ForecastHour[];
  /** "dashboard" is four tiles and one chart; "compact" is tiles only. */
  ...
  const swellSeries = hours.map((h) => ({ t: h.hour, value: Number(h.swell.toFixed(2)) }));
  ...
  const tiles = [
    ...
    },
    {
      ...
      tone: ekman > 0 ? ("cool" as const) : ("warm" as const),
    },
    {
      label: "Coriolis f",
    ...
    },
    {
      ...
      tone: steepness > 0.04 ? ("warm" as const) : ("neutral" as const),
    },
    {
      label: "Air–sea ΔT",
      value: `${airSea >= 0 ? "+" : ""}${airSea.toFixed(1)} °C`,
      hint: airSea > 0 ? "Sea warmer — unstable, fluxes upward" : "Sea cooler — stable layer",
      tone: airSea > 0 ? ("warm" as const) : ("cool" as const),
    },
    {
  ...
  return (
    <section className="space-y-3">
      ...
      <p className="flex items-start gap-1.5 rounded-lg border border-zinc-200 bg-zinc-50 px-2.5 py-2 text-[10px] leading-relaxed text-zinc-600">
        ...
        <span>
          Computed on this device from the conditions above using the relation
          printed under each value. These are diagnostics, not measurements —
          cite the ERDDAP series instead.
        </span>
      </p>
      ...
      {variant === "dashboard" && (
        ...
        >
          <TimeSeriesChart points={waveSeries} baseline={swellSeries} unit="m" height={180} />
        </ChartCard>
      )}

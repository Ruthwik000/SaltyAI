...
/**
 * Fleet vessel telemetry view (coastal operator).
 *
 * Preserved verbatim from the pre-mobile-rebuild page so the operator console
 * keeps the exact view it had. The fisherman role now routes to its own live
 * trip tracker instead.
 */
...
export function VesselTelemetryView() {
  ...
  const [selectedVessel, setSelectedVessel] = React.useState(activeVessels[0]);
  ...
  const handleToggleSos = () => {
    ...
    if (next) {
      alert(
        `EMERGENCY SOS BEACON BROADCASTED to Indian Coast Guard MRCC for vessel ${selectedVessel.name}!`
      );
    }
  };
  ...
  const breadcrumbs = [
    {
      time: "10 mins ago",
      lat: (selectedVessel.currentLat - 0.02).toFixed(3),
      lon: (selectedVessel.currentLon - 0.03).toFixed(3),
      sog: selectedVessel.sogKnots,
      wave: "1.6m",
      note: "Cruising speed",
    },
    {
      time: "30 mins ago",
      lat: (selectedVessel.currentLat - 0.06).toFixed(3),
      lon: (selectedVessel.currentLon - 0.08).toFixed(3),
      sog: selectedVessel.sogKnots + 0.5,
      wave: "1.5m",
      note: "Heading ENE",
    },
    {
      time: "1 hour ago",
      lat: (selectedVessel.currentLat - 0.12).toFixed(3),
      lon: (selectedVessel.currentLon - 0.15).toFixed(3),
      sog: selectedVessel.sogKnots + 0.8,
      wave: "1.4m",
      note: "Departed Outer Channel",
    },
    {
      time: "2 hours ago",
      lat: (selectedVessel.currentLat - 0.22).toFixed(3),
      lon: (selectedVessel.currentLon - 0.26).toFixed(3),
      sog: "4.2",
      wave: "1.2m",
      note: "Harbor exit cleared",
    },
  ];
              ...
              <div className="p-3 rounded-lg border border-zinc-200 bg-zinc-50">
                ...
                <span
                  className={`font-bold text-sm ${
                    selectedVessel.distanceToIMBLNM < 10
                      ? "text-amber-700"
                      : "text-zinc-950"
                  }`}
                >
            ...
            <CardContent className="pt-4 space-y-3 font-sans text-xs">
              ...
              <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
                ...
                <span className="font-bold text-zinc-900">
                  {location.waveHeight}m ({location.wavePeriod}s)
                </span>
              </div>
              ...
              <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
                ...
                <span className="font-bold text-zinc-900">
                  {location.windSpeed} kts {location.windDirection}
                </span>
              </div>
              ...
              <div className="pt-3">
                <Link href="/app/map">
                  <Button
                    variant="outline"
                    size="sm"
                    className="w-full text-xs h-8 border-zinc-200"
                  >
                    Track on Marine Map →
                  </Button>
                </Link>

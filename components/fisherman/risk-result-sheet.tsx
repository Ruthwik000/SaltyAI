...
interface RiskResultSheetProps {
  open: boolean;
  loading: boolean;
  result: TripRiskResult | null;
  /** "demo" means the score was worked out locally, not by the risk model. */
  source: DataSource;
  /** Why the assessment could not be produced, when result is null. */
  reason?: string;
  destinationName: string;
  /** Saves this trip to the fisherman's planned trips. */
  onAddToTrips?: () => void;
  /** True once this assessment has been saved. */
  added?: boolean;
  onClose: () => void;
}
          ...
          <div className="space-y-4 px-4 py-8">
            <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3">
              ...
              <div className="text-xs leading-relaxed text-amber-900">
                ...
                The SALTY risk service did not respond, so there is no score for
                this trip. Do not treat that as a safe result — check the official
                IMD and INCOIS bulletins before you sail.
                {reason && (
            ...
            {source === "demo" && (
              <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
                ...
                <p className="text-[11px] leading-relaxed text-amber-900">
                  <strong>Demo estimate.</strong> The SALTY risk model is not
                  reachable, so this score was worked out on this device from the
                  bundled sample conditions. Do not sail on it — check the

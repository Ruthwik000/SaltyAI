...
export function ZonesMapView() {
  ...
  const [zonesSource, setZonesSource] = React.useState("demo");
  const [zonesReason, setZonesReason] = React.useState();

  const [selectedZoneId, setSelectedZoneId] = React.useState(null);
  const [detail, setDetail] = React.useState(null);
  const [detailSource, setDetailSource] = React.useState("demo");
  ...
  const [loadedCoastKey, setLoadedCoastKey] = React.useState(null);
  const zonesLoading = loadedCoastKey !== coastKey;

  const [loadedZoneId, setLoadedZoneId] = React.useState(null);
  const detailLoading = Boolean(selectedZoneId) && loadedZoneId !== selectedZoneId;
  const shownDetail = detail && detail.zoneId === selectedZoneId ? detail : null;
  ...
  const handleSelect = React.useCallback((zoneId) => {
        ...
        <div className="relative h-full flex-1">
          ...
          <div
            className={`absolute inset-x-0 bottom-0 z-20 flex flex-col rounded-t-2xl border-t border-zinc-200 bg-white shadow-[0_-8px_24px_rgba(0,0,0,0.10)] transition-[height] duration-200 lg:hidden ${
              !sheetOpen ? "h-14" : selectedZoneId ? "h-[78%]" : "h-[56%]"
            }`}
          >
            <button
              type="button"
